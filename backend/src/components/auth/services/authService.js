...
} from '../../email/services/emailService.js'
...
export const signIn = async ({ email, password }) => {
  ...
  if (!user.emailVerified) {
    ...
    throw createError(
      'Por favor, verifica tu email antes de iniciar sesion',
      400
    )
  }
...
export const requestPasswordReset = async (email) => {
  ...
  if (!user) {
    logger.error(
      `Error en requestPasswordReset: Usuario con email: ${email} no encontrado`
    )
