...
export const deleteActivityCtrl = async (req, res) => {
  ...
  try {
    ...
    logger.info(`Actividad con id: ${id} eliminada para el usuario con id: ${userId}`);
  } catch (error) {
    logger.error(`Error al eliminar la actividad con id: ${id} para el usuario con id: ${userId}. Su error es: ${error.stack}`);
    res.status(error.statusCode || 500).json({
      message:
        error.statusCode === 404
          ? 'No se encontró la actividad'
